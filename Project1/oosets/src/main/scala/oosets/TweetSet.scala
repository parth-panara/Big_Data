...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    def maxRetweeted(tweet1: Tweet, tweet2: Tweet): Tweet = if (tweet1.retweets < tweet2.retweets) tweet2 else tweet1

    val lmrt = try {
      left.mostRetweeted
    } catch {
      case e1: NoSuchElementException => elem
    }

    val rmrt = try {
      right.mostRetweeted
    } catch {
      case e2: NoSuchElementException => elem
    }

    maxRetweeted(elem, maxRetweeted(lmrt, rmrt))
  }
